...
func loadParams(param string) string {
	stdout, err := cmd.Execf("grep '^%s=' /usr/bin/pxctl | cut -d'=' -f2", param)
	if err != nil {
		panic(err)
	}
	info := strings.ReplaceAll(stdout, "\n", "")
	if len(info) == 0 || info == `""` {
		panic(fmt.Sprintf("error `%s` find in /usr/bin/pxctl", param))
	}
	return info
}

func loadChange() bool {
	stdout, err := cmd.Exec("grep '^CHANGE_USER_INFO=' /usr/bin/pxctl | cut -d'=' -f2")
	if err != nil {
		return false
	}
	return stdout == "true\n"
